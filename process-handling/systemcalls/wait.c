...
int main() {
  ...
  int status1, status2;
  int childToWait1 = wait(&status1);
  printf("Parent: Child with PID %d has finished with status %d\n", childToWait1, WEXITSTATUS(status1)); // pay attention that status has extra information, so a macro WEXITSTATUS is needed to keep only the exit status
  int childToWait2 = wait(&status2);
  printf("Parent: Child with PID %d has finished with status %d\n", childToWait2, WEXITSTATUS(status2)); // pay attention that status has extra information, so a macro WEXITSTATUS is needed to keep only the exit status
